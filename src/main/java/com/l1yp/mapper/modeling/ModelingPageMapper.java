...
public interface ModelingPageMapper extends BaseMapper<ModelingPage> {
    ...
    @Select("SELECT page.* FROM modeling_page_ref ref LEFT JOIN modeling_page page ON ref.page_id = page.id WHERE page.module = #{module} AND page.mkey = #{mkey} AND ref.page_key = #{pageKey} ")
